...
export class CloneWorker extends WorkerHost {
  ...
  async process(job: Job<CloneJobData>): Promise<void> {
    ...
    const cloneBasePath =
      this.configService.get<string>("ingestion.cloneBasePath") ?? "/tmp/repos";
    const projectPath = `${cloneBasePath}/${serviceId}`;

    this.logger.log(
      `Starting clone for repository ${repo.name} (${repositoryId}) → ${projectPath}`,
    );
    ...
    try {
      if (fs.existsSync(projectPath)) {
        ...
        this.logger.log(
          `Cloning ${gitUrl} branch=${branch} into ${projectPath}`,
        );
        await simpleGit().clone(gitUrl, projectPath, [
          "--branch",
          branch,
          "--single-branch",
        ]);
      ...
      this.logger.log(
        `Clone succeeded for ${repo.name} (${repositoryId}), extract job queued`,
      );
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      this.logger.error(
        `Clone failed for ${repo.name} (${repositoryId}): ${message}`,
      );
