...
export class WebhookService {
  ...
  async handlePush(repoUrl: string, branch: string): Promise<{ repositoryId: string }> {
    const repo = await this.repoRepository.findOne({
      where: { gitUrl: repoUrl, branch },
    });

    if (!repo) {
      throw new NotFoundException(`No registered repository found for ${repoUrl} on branch ${branch}`);
    }

    await this.repoRepository.update(repo.id, { status: RepositoryStatus.CLONING });

    await this.cloneQueue.add('clone', { repositoryId: repo.id, serviceId: repo.serviceId });

    this.logger.log(`Ingestion triggered for repository ${repo.name} (${repo.id})`);
